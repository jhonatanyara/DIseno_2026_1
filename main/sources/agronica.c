...
void set_actuator_state(gpio_num_t pin, bool enable) {
    ...
    if (pin == PIN_VALVULA) {
        valve_state = enable;
    } else if (pin == PIN_DOSIFICADOR) {
        dosificador_state = enable;
    }
}
...
void iniciar_hardware(void) {
    ...
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << PIN_VALVULA) | (1ULL << PIN_DOSIFICADOR),
    ...
    set_actuator_state(PIN_VALVULA, false);
    set_actuator_state(PIN_DOSIFICADOR, false);
